...
| F9 | 远程「binary 路径选择」策略落地：本机窗口用服务端原生 pick（rfd）；远程浏览器支持手填 + 上传二进制（`/api/upload`，受 F6 鉴权，限 512MB，存 `<data_dir>/uploads/` 回填绝对路径） | 本机原生选路径、远程可手填或直接上传 | ☑ |
...
## 阶段 G — 移动端独立展示方案（卡片流 + 二级页面）

> 目标：移动端不再压缩桌面布局，而是走一套独立的「卡片流 + 二级页面」设计（方案 B）。程序列表以卡片纵列展示，点卡片进入独立管理详情页；批量/模板库/日志/设置各自成为独立页面，用底部 Tab 导航切换。与桌面端共用同一份 SPA 与 RPC/WS 数据层，仅 UI 结构独立（`@media (max-width:720px)` 断点 + 移动专用容器）。

| # | 任务 | 验收标准 | 状态 |
|---|---|---|---|
| G1 | 移动端底部 Tab 导航栏（程序/批量/模板库/日志中心/设置），程序列表改为卡片纵列流 | 手机握持拇指可切换五大视图；卡片显示图标+名称+状态+快捷启停 | ☑ (底部 Tab + 卡片流，设置经顶部 ⚙ 打开设置弹窗) |
| G2 | 点卡片进入程序管理二级页（详情字段 + 启停/重启 + 应用目录/复制/打开网站 + 内嵌日志） | 页内操作完整可用、返回可回到卡片列表 | ☑ (复用 manage-view 全屏 + 返回栏) |
| G3 | 批量/模板库/日志中心移动专用布局（表格改卡片/列表流、日志全屏、编辑/导入抽屉式全屏弹窗） | 三视图在手机上一屏信息密度合理、可操作 | ☑ (批量改卡片流+启停/编辑/删除/下载操作；模板库卡片式、导入按钮加大；日志工具栏全宽搜索、触控加大；编辑/导入/设置/源管理弹窗底部抽屉式全屏) |
| G4 | 移除桌面端之前为移动显示打的冗余补丁（顶部工具条式侧栏/双层布局/行动作区溢出补丁等） | 移动端仅走 G 方案样式，无残留冲突 | ☑ |
| G5 | 移动端 e2e 手测（iOS/Android 浏览器）+ 视觉回归 | 启停/查看/编辑在真机可完成 | ☐ |

## 执行顺序建议

A（1→7）→ B1-B3 → B4/B7（B8）→ B9 → C1/C3 → C4/C2/C5 → C6 → D1-D3 → F-1（F1→F2→F3/F4→F5）→ F-2 → F-3 → G-1（G1→G2→G3→G4→G5）
...
- **移动端方案**：移动端采用独立「卡片流 + 二级页面」设计（方案 B），不压缩桌面布局；底部 Tab 导航 + 程序卡片流 + 独立详情页；与桌面共用同一 SPA 与 RPC/WS 层，仅 UI 结构独立（`@media (max-width:720px)` 断点）