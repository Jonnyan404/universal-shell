...
| F2 | SPA 单一起源 `frontend/`：收纳 app-tauri 的 HTML/CSS/JS；新增 `bridge.js`（Tauri `invoke` 与 fetch/WebSocket 双通道自动探测），渲染逻辑复用现有 main.js | 同一份前端在浏览器与 Tauri 窗口中行为一致 | ☐ |
...
| F4 | tauri 内嵌：启动即起服务（绑定 127.0.0.1），窗口加载改为主机 localhost=同一 SPA；托盘加「在浏览器打开」；原生能力（reveal/文件对话框）保留为 bridge 的 invoke fallback | 桌面窗口与浏览器访问同一前端、同一服务 | ☐ |
...
| F8 | 模板浏览/导入、批量管理、设置页全量迁移到桥接层，与桌面端功能对齐 | 功能清单逐项比对无缺 | ☐ |

### F-3 体验优化（事件推送）

| # | 任务 | 验收标准 | 状态 |
|---|---|---|---|
| F9 | shared 增加轻量事件订阅（状态/日志变更回调）→ WS 推送到浏览器；egui/tauri 桌面端也改用事件替换轮询 | 外部 kill 状态 <1s 到达页面，无需轮询 | ☐ |
| F10 | 大日志增量推送（tail diff），不整页重传 | 长运行日志前端不卡顿 | ☐ |
| F11 | 移动端响应式布局，手机浏览器可启停/查看 | iOS/Android 浏览器可用 | ☐ |
...
- **Web 界面**：内嵌进 Tauri 与 egui 两进程、不单独打包；SPA 单一起源 `frontend/`，浏览器与 Tauri 窗口共享同一前端、同一服务；`bridge.js` 探测 Tauri invoke 作原生能力 fallback
