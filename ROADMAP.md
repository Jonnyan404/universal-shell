...
> 目标：新用户首次打开软件时，看到一个轻量引导说明，快速了解核心操作（模板库导入 → 启停 → 设置入口），之后不再出现。

### 需要解决的问题

- 用户首次打开对界面零认知：不知从哪开始、不知道要先导入模板
- 不想做复杂交互系统，只要一个「能看就懂、点掉就忘」的简单引导
...
**交互**
1. 首次启动后（检测标记文件不存在）立即弹出一个半屏模态窗
2. 内容：3-4 个步骤卡片（可左右滑动/点 Next）：
   - 「欢迎使用」→ 简述软件定位（配置驱动的万能应用壳）
   - 「导入模板库」→ 附截图说明「模板库」标签页的使用
   - 「启停程序」→ 附截图说明运行/停止按钮位置
   - 「全局设置」→ 简介设置入口（代理、自启等）
3. 底部有「跳过」+「开始使用」按钮；点任一按钮关闭弹窗并写标记

**存储**
- 标记文件：`<data_dir>/first_run_done`（空文件即可，或写 `{"shown": true}` JSON）
- egui：同目录（ShellManager.data_dir 是共享的）

**实现点（Tauri/web）**
- `frontend/index.html`：新增 `<div id="onboarding-modal">` 弹窗 DOM
- `frontend/main.js`：启动时调 `GET /api/get_shell_config` 或新增 RPC `is_first_run`；是→显示弹窗；点关闭→调 `POST /api/ack_first_run`
- `web-server/src/rpc.rs`：新增 `is_first_run`（检查文件存在）和 `ack_first_run`（touch 文件）
- `shared/src/config.rs`：无新字段（标记文件独立于 shell.json）

**实现点（egui）**
- `app-egui/src/main.rs`：启动时检测 first_run_done 文件；弹 egui::Window 引导弹窗；点关闭/确认后写文件

**优点**
- 最直观、用户理解成本为零
- 内容可复制文案 + 静态示意图（无截图依赖，避免不同 UI 主题下截图失真）
- 首次可见、关闭后永久隐藏、极简存储

**缺点**
- 弹窗打断主界面渲染顺序（需确保主界面先加载好再弹）
- 多语言需两份 locale 文案
...
**交互**
1. 首次启动后，依次在关键元素旁弹出箭头/气泡提示（5-6 步）
2. 每步提示带「下一步」和「跳过」按钮；点完最后一步或点跳过→关闭并写标记
3. 每次只高亮一个元素，其余界面灰化（overlay）

**存储**
- 同方案 A：`<data_dir>/first_run_done`

**实现点（Tauri/web）**
- `frontend/main.js`：新增一个轻量 tour 引擎（纯 JS/CSS），监听元素位置，计算箭头方向
- 依赖固定的 DOM id（#manage-btn、#batch-btn、#lib-btn 等）；元素不可见或不存在时自动跳过
...
**缺点**
- 实现复杂度显著更高（需要 overlay、定位计算、动画、响应式适配）
- 移动端/小屏下定位不准，容易遮挡或错位
- DOM id 变动时提示错位，维护成本高
...
**交互**
1. 主界面侧栏或右上角新增一个「使用说明」图标按钮（ℹ 或 ❓）
2. 点击后展开一个侧栏或模态，内含纯文字/静态 HTML 说明
3. 首次启动自动展开；用户可关闭；之后手动点击再次打开

**存储**
- 标记文件同上；另外主界面保留常驻入口

**实现点**
- `frontend/index.html`：新增说明面板 DOM
- `frontend/styles.css`：侧栏或弹窗样式
- `frontend/main.js`：自动展开逻辑 + 手动触发按钮
...
把引导文案写进 `README.md` 或 `docs/usage-guide.md`，软件内不加 UI。

**优点**：零代码改动，维护成本最低

**缺点**：用户几乎不会在软件内看文档；不解决「首次打开不知从哪开始」问题
...
理由：
1. 解决核心问题（首次看到即引导），实现简单
2. 存储极简（单个标记文件，不改 ShellConfig schema）
3. 对移动端/响应式友好（模态自适应）
4. 对 egui/tauri 双端：各自加一个同构弹窗，共用标记文件
5. 后续可扩展（卡片内容通过 locale 管理、支持多语言、后续可升级为视频/动画）

### 待确认

1. 弹窗步骤数量和内容（当前建议 4 步，可增减）
2. 是否需要「查看使用说明」常驻入口（即方案 A+C 结合）
3. 多语言文案范围（zh-CN + en 是否够）