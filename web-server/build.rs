...
//! 在构建时转换为 `frontend/locales/*.json` 供浏览器 fetch 使用。
...
fn main() {
    ...
    let out_dir: PathBuf = Path::new(env!("CARGO_MANIFEST_DIR")).join("../frontend/locales");
    fs::create_dir_all(&out_dir).expect("create frontend/locales");
    ...
    if let Ok(rd) = fs::read_dir(&locales_dir) {
        for e in rd.flatten() {
            ...
            let flat: serde_json::Map<String, serde_json::Value> = obj
                ...
                .collect();
            let file_stem = path.file_stem().expect("filename").to_string_lossy().to_string();
